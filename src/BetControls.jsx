...
function BetButtons({ id }) {
      ...
      if (!autoCashOut) {
        ...
        if (user?.money > extraBetAmount && !isBetPlaced) {
          ...
          if (res.status) {
            ...
            if (id === 1) {
              localStorage.setItem("betId1", res.betId.toString());
            } else {
              localStorage.setItem("betId2", res.betId.toString());
            }
            if (socket) {
  ...
  // console.log(planeValue, autoCashOutValue);
  useEffect(() => {

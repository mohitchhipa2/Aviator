...
import { fetchData, generateTransactionId } from "../api/ClientFunction";
import { useLocation } from "react-router-dom";
import DepositeAmount from "./DepositeAccount";
import useSWR from "swr";
const Deposit = () => {
  const [show, setShow] = useState(0);
  const { data, loading } = useSWR("/admin/gatewaykey", fetchData)
  console.log(data)
  useEffect(() => {
    if (data && data.data) {
    ...
    <div>
      ...
      >
        <form
          ...
          style={{ color: "white", display: "flex", flexDirection: 'column', alignItems: 'center', fontFamily: "monospace", marginBottom: "20px" }}
        >
          <h2>Deposit</h2>
